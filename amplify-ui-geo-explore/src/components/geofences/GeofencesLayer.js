...
const GeofencesLayer = ({ client, isOpenedPanel, onPanelChange, isDrawing, onDrawingChange }) => {
        ...
        <InfoBox header="Geofence Drawing Mode">
          {isAddingGeofence ? (
            ...
            <>
              ...
              <p>
                Click on the initial point to complete the geofence. A geofence must have at least 3
                points.
              </p>
          ...
          <Button variation="primary" size="small" onClick={() => onDrawingChange(false)}>
            Exit
          </Button>
        </InfoBox>
      )}
      {geofences?.length > 0 && (
        <DrawnGeofences geofences={geofences} geofencesVisible={geofencesVisible} />
      )}
    </>
  );
};
