...
import { MAP_CONTAINER, TRACKERS_PANEL } from "../../constants";

// Get all devices in the collection
const callListDevicePositionsCommand = async (client) => {
  if (client) {
...
const callBatchUpdateDevicePositionCommand = async (client, position) => {
  if (client) {
    const command = new BatchUpdateDevicePositionCommand({
      TrackerName: TRACKER,
      Updates: [
        {
          DeviceId: TRACKER_SIMULATED_DEVICE,
          Position: position,
          PositionProperties: {
            // Sample properties associated with the position update
            DriverId: "321",
            VehicleSpeed: `${Math.floor(Math.random() * 60)} km/h`,
          },
          SampleTime: new Date(),
        },
      ],
...
// Get device history
const callGetDevicePositionHistoryCommand = async (client, deviceId) => {
  if (client) {
    ...
    const command = new GetDevicePositionHistoryCommand({
      ...
      StartTimeInclusive: new Date(currentTime.getTime() - 60 * 60 * 1000),
    });
...
// Adding and removing crosshair cursor
const showCrosshairCursor = (state) => {
  if (state) {
    document.querySelector(`#${MAP_CONTAINER}`).classList.add("crosshair-cursor");
  } else {
    document.querySelector(`#${MAP_CONTAINER}`).classList.remove("crosshair-cursor");
  }
};
...
const TrackersLayer = ({
  ...
  const fetchDevicePositions = async () => {
    const fetchedDevices = await callListDevicePositionsCommand(client);
    setDevices(fetchedDevices.Entries);
    // Clear clicked location of the simulated marker
    setSimulatedPosition();
  };

  const fetchDevicePositionHistory = async (deviceId) => {
    const fetchedDeviceHistory = await callGetDevicePositionHistoryCommand(client, deviceId);
  ...
  useEffect(() => {
    // Fetch devices when the app loads
  ...
  useEffect(() => {
    if (isOpenedPanel) {
      // Fetch device positions when the trackers panel opens
      fetchDevicePositions();
      if (!isViewingDeviceHistory) {
        // Change cursor to crosshair when panel opens and is not viewing device history
        showCrosshairCursor(true);
      }
    } else {
      // Remove crosshair cursor when panel closes
  ...
  useEffect(() => {
    if (isViewingDeviceHistory) {
      // Remove crosshair cursor when viewing device history
      showCrosshairCursor(false);
      // Fetch device position history of the specified device
      fetchDevicePositionHistory(deviceIdHistory);
    } else {
      if (isOpenedPanel) {
        // Change cursor to crosshair when exiting device position history viewer while Trackers panel is open
        showCrosshairCursor(true);
      }
  ...
  // Update device position when the map is clicked while simulating a device
  useEffect(() => {
    // Updating device position is disabled when viewing device positions
    if (clickedLngLat && isOpenedPanel && !isViewingDeviceHistory && !deviceIdHistory) {
      setSimulatedPosition({
        longitude: clickedLngLat.lng,
        latitude: clickedLngLat.lat,
      });
      const updateDevicePosition = async () => {
        try {
          await callBatchUpdateDevicePositionCommand(client, [
            clickedLngLat.lng,
            clickedLngLat.lat,
          ]);
        } catch {
          alert("There was an error updating the device position.");
        }
      };
    ...
    <>
      ...
      {isOpenedPanel && (
        <TrackersPanel
          ...
          onToggleDevices={() => setDevicesVisible((prev) => !prev)}
          simulatedPosition={simulatedPosition}
          isViewingDeviceHistory={isViewingDeviceHistory}
        />
      )}
      {simulatedPosition && (
        <Marker longitude={simulatedPosition.longitude} latitude={simulatedPosition.latitude}>
          <svg width="8" height="8" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <circle cx="50" cy="50" r="50" fill="#7AC943" />
          </svg>
        </Marker>
      )}
      {devicesVisible && !isViewingDeviceHistory && !deviceIdHistory && (
