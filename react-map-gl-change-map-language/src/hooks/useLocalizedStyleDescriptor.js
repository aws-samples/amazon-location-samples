...
const useLocalizedStyleDescriptor = (geoConfig, language) => {
        ...
        try {
          const res = await locationClient.current.send(
            new GetMapStyleDescriptorCommand({
              MapName: geoConfig.mapName[0],
            })
          );
          if (res.Blob.length === 0) {
            throw new Error("No style descriptor found");
          }
          const styleDescriptorJson = JSON.parse(
            textEncoder.current.decode(res.Blob)
          );

          const isEsriStyle =
            geoConfig.style[0].toUpperCase().indexOf("ESRI") > -1;
          if (isEsriStyle) {
